...
def process_row(row):
    ...
    if isinstance(eval_result, Ielt):
        return {
            "band": overall_band,
            "prompt": essay_prompt,
            "essay": essay_text,
...
def process_chunk_parallel(chunk, max_workers=5):
    """Xử lý 1 chunk song song với nhiều thread"""
    results = []
...
def process_csv_in_batches(input_csv, output_csv, batch_size=10, start_row=0, max_batches=None, max_workers=5):
    """
    ...
    - max_workers: số luồng song song để chấm điểm
    """
    ...
    for chunk in reader:
        ...
        print(f"Đang xử lý batch {batch_count} ({len(chunk)} essays) với {max_workers} luồng...")
        results = process_chunk_parallel(chunk, max_workers=max_workers)
...
# --- Chạy gen file ---
process_csv_in_batches("test.csv", "new_test.csv", start_row=250, batch_size=10, max_batches=25, max_workers=10)